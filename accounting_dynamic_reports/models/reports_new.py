...
class AccountCommonReportNew(models.TransientModel):
    ...
    target_move = fields.Selection([('all', 'All'), ('posted', 'Posted')], string="Type", default='all')
    ...
    debit_credit = fields.Boolean(string='Display Debit/Credit Columns', default=True, help="This option allows you to get more details about the way your balances are computed. Because it is space consuming, we do not allow to use it while doing a comparison.")
        ...
        for i in report_lines:
            if i['type'] == 'account':
                ...
                if form['target_move'] == 'posted':
                    search_query = "select aml.id, am.id as j_id, aml.account_id, aml.date, aml.name as label, am.name, " \
                        + "(aml.debit-aml.credit) as balance, aml.debit, aml.credit, aml.partner_id " \
                        + " from account_move_line aml join account_move am on (aml.move_id=am.id and am.state=%s) " \
    ...
    def view_report(self):
        ...
        data['form'] = self.read(['date_from', 'enable_filter', 'debit_credit', 'date_to', 'account_report_id', 'target_move', 'view_format'])[0]
